(() => {
  ...
  function updateActionAvailability() {
    ...
    ["previewButton", "addBpmSuffixButton", "renameGroupButton", "renameAllButton", "importExcel", "applyMappingButton", "parsePreviewButton"].forEach((id) => { $(id).disabled = disabled; });
  }
  ...
  function applyState(next) {
    ...
    Object.keys(extensions).forEach((ext) => {
      if (!(ext in state.selectedExtensions)) state.selectedExtensions[ext] = true;
    });
  ...
  function render() {
    ...
    $("currentGroupMetric").textContent = group ? `${group.relative_folder || group.folder_name} / ${group.extension.replace(".", "").toUpperCase() || "无扩展名"}` : "-";
    $("scanSummary").textContent = data.root ? `${data.root} · ${data.total_file_count ?? (data.records || []).length} 个文件` : "等待扫描";
    if (group) {
      $("metaPrefix").value = data.meta_prefix || "";
      $("groupPrefix").value = group.prefix || "";
      const records = currentRecords().filter((record) => !record.removed);
      const commonChild = records.length && records.every((record) => record.child_prefix === records[0].child_prefix) ? records[0].child_prefix : "";
      $("childPrefix").value = commonChild;
    }
  ...
  function renderGroups() {
    ...
    container.innerHTML = groups.map((group) => `
      <div class="group-row ${group.key === state.data.current_group_key ? "active" : ""}" data-group-key="${esc(group.key)}">
        ...
        <span class="group-text" title="${esc(group.folder)}">${esc(group.relative_folder || group.folder_name)} / ${esc(group.extension.replace(".", "").toUpperCase() || "无扩展名")}</span>
  ...
  function renderRows() {
    ...
    if (!rows.length) { body.innerHTML = '<tr><td colspan="12" class="empty-row">当前筛选没有文件</td></tr>'; $("selectAll").checked = false; $("selectAll").indeterminate = false; updateSelectedMetric(); return; }
    body.innerHTML = rows.map((record) => {
      ...
      const bpmField = record.bpm ? `BPM: ${record.bpm}` : "";
      const parsedFields = [bpmField, ...Object.entries(record.parsed_fields || {}).filter(([key, value]) => value && key !== "bpm").map(([key, value]) => `${key}: ${value}`)].filter(Boolean).join(" · ");
      ...
      const nameLocked = state.data?.mode !== "original" || state.data?.parse_use_name;
      return `<tr draggable="true" class="${rowClass}" data-path="${esc(record.path)}">
        ...
        <td><input class="inline-input" data-record-field="child_prefix" value="${esc(record.child_prefix)}"></td>
        <td><input class="inline-input" data-record-field="name" value="${esc(record.name)}" ${nameLocked ? 'disabled title="当前模式由编号、Excel 或解析结果生成名称"' : ""}></td>
  ...
  function previewPayload() {
    const group = currentGroup();
    return { group_key: group?.key, meta_prefix: $("metaPrefix").value, group_prefix: $("groupPrefix").value, child_prefix: $("childPrefix").value, separator: $("separator").value, mode: $("nameMode").value, parse_template: $("parseTemplate").value, parse_use_name: $("parseUseName").checked,
  ...
  async function exportTables() {
    ...
    try {
      ...
      showToast(`导出完成：${xlsxOutputs.length} 个 XLSX${structure ? "，已生成结构文件" : ""}`);
  ...
  function bindEvents() {
    ...
    ["metaPrefix", "groupPrefix", "childPrefix", "separator", "numericStart", "numericWidth", "numericStep"].forEach((id) => $(id).addEventListener("change", () => { state.taskDirty = true; refreshPreview(); }));
    ...
    $("addBpmSuffixButton").addEventListener("click", async () => {
      try {
        if (!await flushRecordUpdates()) return;
        const group = currentGroup();
        if (!group || !requireCurrentScan()) { if (!group) showToast("请先扫描并选择命名组", true); return; }
        const payload = await api("/api/add-bpm-suffix", jsonOptions({ group_key: group.key }));
        state.taskDirty = true;
        applyState(payload.state);
        showToast(`BPM 后缀已添加 ${payload.added} 个${payload.missing ? `，${payload.missing} 个未识别 BPM` : ""}`, payload.missing > 0);
      } catch (error) { showToast(error.message, true); }
    });
